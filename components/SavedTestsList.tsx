...
// Inline confirmation button for deleting a saved test
const ConfirmDeleteButton: React.FC<{ onDelete: () => void }> = ({ onDelete }) => {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className={`p-2 rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-white/50 focus-visible:ring-offset-2 focus-visible:ring-offset-stitch-dark ${
        confirming
          ? 'text-white bg-red-600 hover:bg-red-700 font-bold text-xs px-3'
          : 'text-stitch-muted hover:text-red-400 hover:bg-red-500/10'
      }`}
      aria-label={confirming ? "Confirm Delete" : "Delete Test"}
      title={confirming ? "Confirm Delete" : "Delete Test"}
    >
      {confirming ? (
        "Sure?"
      ) : (
                ...
                <div key={test.id} className="bento-card overflow-hidden group hover:border-white/30 transition-all flex flex-col text-left">
                    ...
                    <div className="p-3 bg-white/5 border-t border-white/10 flex gap-2 justify-between">
                         <div className="flex gap-2 w-full">
                            ...
                            <ConfirmDeleteButton onDelete={() => onDelete(test.id)} />
                         </div>
                    </div>
                </div>
              );
