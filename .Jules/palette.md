...
## 2024-05-19 - Inline Delete Confirmation
**Learning:** Found that destructive actions like deleting a saved test lacked confirmation, risking data loss. While modals add friction, an inline confirmation pattern ('Sure?') provides safety without heavy cognitive load or interrupting the flow.
**Action:** When adding delete actions for list items, implement an inline confirmation state (with a timeout and blur dismissal) rather than immediate deletion or full modals.
