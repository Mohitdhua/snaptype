...
## 2024-05-19 - Focus States for Hidden Interactive Elements
**Learning:** File inputs or other interactive elements styled with `opacity-0` (often used for custom dropzones) become completely invisible to keyboard navigation because their native focus rings are also hidden. Users relying on keyboards have no idea when these elements have focus.
**Action:** When implementing hidden or opacity-0 inputs, always apply `:focus-within` styles (e.g., `focus-within:ring-2`) to the visible parent container to maintain keyboard focus visibility.
