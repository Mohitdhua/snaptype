...
## 2024-07-28 - Hidden File Input Focus Accessibility
**Learning:** Visually hiding (`opacity-0`) native file inputs while keeping them in the DOM creates an accessibility issue where keyboard users cannot see the focus indicator when tabbing to the input.
**Action:** Always apply `:focus-within` styles to the visible parent wrapper element of the hidden input so the focus state remains visually clear to keyboard users.
