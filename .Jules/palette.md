...
## 2024-10-24 - Inline Confirmation for Destructive Actions
**Learning:** Found that system modals (like `window.confirm`) for destructive actions such as deleting saved tests interrupt the flow and feel jarring. Using an inline two-tap confirmation (e.g., transforming a "Trash" icon to a "Sure?" button with a timeout) provides a smoother and more integrated user experience without sacrificing safety.
**Action:** Prefer implementing inline, timeout-based confirmation states over blocking browser modals for non-critical destructive actions to maintain UI flow.
