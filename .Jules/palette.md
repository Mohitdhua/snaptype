...
## 2024-08-09 - Inline Confirmations Over System Modals
**Learning:** System modal dialogs (like `window.confirm`) disrupt the user's workflow by halting interaction until addressed.
**Action:** Replace destructive action confirmations with inline state changes (e.g., transforming a "Delete" button into a "Sure?" prompt with a timeout) to maintain flow while still protecting against accidental clicks.
